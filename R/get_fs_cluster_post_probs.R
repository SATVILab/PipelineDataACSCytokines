...
get_fs_cluster_post_probs <- function(flowsom_freq = flowsom_freq, 
  ...
  if(!'uns' %in% flowsom_freq$stim) return(NULL)
    ...
    clust_tbl <- purrr::map_df(clust_vec, function(clust){
     # print(clust)
      ...
      fs_freq_clust <- try(fs_freq_stim_clust %>%
        ...
        dplyr::mutate(count_pos = count_stim, 
      ...
      mimosa_expr_set <-try(MIMOSA::ConstructMIMOSAExpressionSet(thisdata = fs_freq_clust,
                                                              reference = stim %in% 'uns',
                                                              measure.columns = c("count_pos", "count_neg"),
                                                              other.annotations = c("feature"),
                                                              .variables = plyr::.(SampleID)))
      
      mim_res <- try(MIMOSA::MIMOSA(formula = count_neg + count_pos ~ SampleID|feature,
                                data = mimosa_expr_set,
                                method = "EM"))
      
      # get posterior probabilities
      post_prob_vec <- try(mim_res[[1]]@z[,"z2"])
      if(class(post_prob_vec)[1] == 'try-error') return(tibble::tibble(clust = clust,
                                                            stim = stim,
                                                            SubjectID = NA, 
                                                            VisitType = NA, 
                                                            Progressor = NA,
      ...
      out_tbl_ind <- tibble::tibble(clust = clust, stim = stim, SubjectID = subjectid_vec, VisitType = visittype_vec, Progressor = prog_lab_vec[subjectid_vec],
             prob = post_prob_lab_vec)
      
      if(all(is.na(out_tbl_ind$prob))){
        return(tibble::tibble(clust = clust,
                      stim = stim,
                      SubjectID = NA, 
                      VisitType = NA, 
                      Progressor = NA,
