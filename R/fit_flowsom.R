...
fit_flowsom <- function(fs, n_clust, scale, chnl_sel, ds_name, dir_save_base, 
  ...
  } else jacc_lab_vec <- NULL # set to NULL if not available
  ...
  if(is.null(jacc_lab_vec) && jacc_n > 0){
    ...
    jacc_lab_vec <- setNames(jacc_tbl_mean$jacc_mean, 
                             jacc_tbl_mean$cluster)
    
    # save scores
    analysispipeline::save_objects(jacc_tbl_mean = jacc_tbl_mean,
                                   jacc_tbl = jacc_tbl,
  ...
  attr(flowsom_out, "par_fs") <- list("n_clust" = n_clust, 'scale' = scale, 
                                      'chnl_sel' = chnl_sel)
