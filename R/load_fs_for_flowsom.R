...
load_fs_for_flowsom <- function(dir_fcs, stim, stim_vec_u = NULL, responders_list, chnl_sel = NULL,
  ...
  fcs_list <- purrr::map(stim_vec_load, function(stim_ind) {
    c(
      list.files(dir_fcs, pattern = paste0(stim_ind, ".fcs")),
  ...
  if (!is.null(responders_list)) {
    ...
    if (length(setdiff(
      setdiff(names(fcs_list), uns_chr),
      setdiff(names(responders_list), uns_chr)
    )) != 0 ||
      length(setdiff(
        setdiff(names(responders_list), uns_chr),
        setdiff(names(fcs_list), uns_chr)
      ...
      message(paste0(
        "names from fcs_list (", paste0(setdiff(names(fcs_list), uns_chr), collapse = " "),
