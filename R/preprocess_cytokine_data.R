#'
#' @param label_cytokines function. A function that takes the cytokine names as input and returns the
...
#' @import ggplot2
#' @import dplyr
preprocess_cytokine_data <- function(gs_nm = "gs_cytof_acs_nk",
                                     chnls = c('Ho165Di', 'Gd158Di', 'Nd146Di',
                                     ...
                                     combn_exc_list = list(NULL, c("Nd146Di", "Gd156Di")),
                                     hladr = FALSE,
                                     hladr_bulk = FALSE,
  ...
  # libraries
  on.exit(theme_set(theme_gray()))
  theme_set(cowplot::theme_cowplot())

  # data
  dir_gs <- normalizePath(file.path(dir_faust, 'faustData',
                                    'gsData', gs_nm))
  #flowWorkspace::convert_legacy_gs(from = dir_gs, to = paste0(dir_gs, "_new"))
  ...
  gate_quant_list = list("min" = c(0.25, 0.5),
                         "no" = c(0.2, 0.35),
                         "prejoin" = c(0.15, 0.25))
  marker_2d_vec <- c('Eu151Di', 'Ho165Di', 'Gd158Di', 'Nd146Di',
                     'Lu175Di', 'Gd156Di', 'Dy164Di', 'Nd150Di')
  marker_2d_vec <- c(marker_2d_vec, "Sm154Di", "Nd145Di",
                     "Er168Di", "Dy161Di", "Gd160Di", "Nd148Di",
                     "Er166Di")
  marker_2d_vec <- setNames(rep(1, length(marker_2d_vec)),
                            marker_2d_vec)

  # actual marker list
  marker_list <- purrr::map(chnls, function(chnl){
    switch(chnl,
           "Ho165Di" = list(cut = "Ho165Di",
                            pop_man_sub = NULL,
                            high = marker_2d_vec,
                            cps_scp = c(90, 95),
                            pop_plot_sub = c(""),
  ...
  fcs_stim_tbl <- purrr::map_df(seq_along(gs), function(i){
    #gh_pop_get_data(gs[[i]])@description$GUID
    fn <- flowCore::description(flowWorkspace::gh_pop_get_data(gs[[i]]))$FILENAME
  ...
  stim_vec_gs <- fcs_stim_tbl$stim
  fcs_lab_vec <- setNames(fcs_stim_tbl$fcs, 1:nrow(fcs_stim_tbl))
  sampleid_lab_vec <- setNames(fcs_stim_tbl$fcs, 1:nrow(fcs_stim_tbl))
  stim_lab_vec <- setNames(fcs_stim_tbl$stim, 1:nrow(fcs_stim_tbl))
  ...
  if(gate || stats_only || plots_only){
    ...
    outliergatev1:::gate(data = gs,
                         ...
                         plot = FALSE,
                         noise_sd = 2,
                         bias_uns = 0.15, # 5/200 - original
                         min_bw = 0.3, # 7/200 - original
                         cp_min = 0,
                         remove_beads = FALSE,
                         boot_n = NULL,
                         ind_skip = NULL,
  ...
  chnl_lab <- outliergatev1:::.get_labs(data = gs[[1]],
                                        cut = purrr::map_chr(marker_list, function(x) x$cut))
  ind_batch_list <- outliergatev1:::.get_ind_batch_list(data = gs, batch_size = 5, fcs = fcs_vec_gs)
  ...
  if(save_cyt_pos_fcs){
    ...
    purrr::walk(combn_exc_list, function(combn_exc){
      purrr::walk(gate_name_vec, function(gn){
        outliergatev1:::get_stim_pos_fcs(data = gs,
                                         data_name = gs_nm,
                                         pop_gate = pop_gate,
                                         chnl =  purrr::map_chr(marker_list, function(x) x$cut),
                                         ind_in_batch_lab_vec = ind_in_batch_lab_vec,
                                         gate_name = gn,
                                         ind_in_batch_gate = 1:5,
                                         cytof_fcs_to_clin_map = NULL,
                                         ...
                                         trans_chnl = c("Dy161Di", "Dy162Di", "Dy163Di", "Dy164Di", "Er166Di", "Er167Di",
  ...
  if(hladr){
    ...
    hladr_tbl <- purrr::map_df(gate_name_vec, function(gate_name){
      ...
      purrr::map_df(chnl_list, function(chnl_vec){
        ...
        purrr::map_df(mult_vec, function(mult){
          print(mult)
          outliergatev1:::get_hladr_med_diff(data = gs,
                                             data_name = gs_nm,
                                             pop_gate = pop_gate,
                                             chnl = chnl_vec, #### MUST ONLY USE IFNG and TNF
                                             mult = mult,
                                             ind_in_batch_lab_vec = ind_in_batch_lab_vec,
                                             gate_name = gate_name,
                                             ind_in_batch_gate = 1:5,
    ...
    analysispipeline::save_objects('hladr' = hladr_tbl,
  ...
  chnl_lab_vec <- c("Ho165Di" = "IFNg", "Gd158Di" = "IL2", "Nd146Di" = "TNF",
                    "Dy164Di" = "IL17", "Nd150Di" = "IL22", "Gd156Di" = "IL6")

  cyt_combn_rep_lab_vec <- purrr::map_chr(unique(stats_combn_tbl$cyt_combn), function(cco){
    ...
    cyt_combn_rep_end <- paste0(pos_ind_lab_vec[pos_ind_vec[-1]], chnl_lab_vec[chnl_vec[-1]], sep = "", collapse = "")
    cyt_combn_rep_start <- paste0(setNames(c("", "!"), c("+", "-"))[pos_ind_vec[1]], chnl_lab_vec[chnl_vec[1]])
    paste0(cyt_combn_rep_start, cyt_combn_rep_end)
  }) %>%
    setNames(unique(stats_combn_tbl$cyt_combn))

  stats_combn_tbl %<>%
    dplyr::mutate(stim = stim_lab_vec[as.character(ind)],
           batch_sh = fcs_lab_vec[as.character(ind)]) %>%
    dplyr::mutate(SubjectID = purrr::map_chr(batch_sh, function(x) str_split(x, "_")[[1]][[1]]),
           VisitType = purrr::map_chr(batch_sh, function(x) str_split(x, "_")[[1]][[2]])) %>%
    dplyr::select(gate_name, ind, batch_sh, SubjectID, VisitType, stim, cyt_combn, count_stim, n_cell_stim, count_uns, n_cell_uns)

  stats_combn_tbl %<>% dplyr::mutate(cyt_combn = cyt_combn_rep_lab_vec[cyt_combn])
  ...
  stats_combn_tbl_neg <- stats_combn_tbl %>%
    ...
    dplyr::summarise(n_cell_stim = n_cell_stim[1],
  ...
  stats_combn_tbl %<>% dplyr::bind_rows(stats_combn_tbl_neg)

  stats_combn_tbl %<>% dplyr::mutate(stim = ifelse(stim == 'mtbaux', 'mtb', stim))
  ...
  if(identical(intersect(chnls, c('Ho165Di', 'Gd158Di', 'Nd146Di')), c('Ho165Di', 'Gd158Di', 'Nd146Di'))){
    ds_name <- paste0(ds_name, "_th1")
  }
  if('Dy164Di' %in% chnls) ds_name %<>% paste0("_il17")
  if('Nd150Di' %in% chnls) ds_name %<>% paste0("_il22")
  if('Gd156Di' %in% chnls) ds_name %<>% paste0("_il6")
  ...
  if(!compass){
    path_compass_old <- file.path(path_cyt_combn_base, "data_out", "compass", 'compass.rds')
    if(file.exists(path_compass_old)) out_list$compass <- readRDS(path_compass_old)
  }

  if(!hladr){
    path_hladr_old <- file.path(path_cyt_combn_base, "data_out", "hladr", 'hladr.rds')
    if(file.exists(path_hladr_old)) out_list$hladr <- readRDS(path_hladr_old)
  } else out_list$hladr <- hladr_tbl


  if(!hladr_bulk){
    path_hladr_old <- file.path(path_cyt_combn_base, "data_out", "hladr_bulk", 'hladr_bulk.rds')
    if(file.exists(path_hladr_old)) out_list$hladr_bulk <- readRDS(path_hladr_old)
  } else out_list$hladr_bulk <- hladr_tbl_bulk

  if(!flowsom){
    path_flowsom_old <- file.path(path_cyt_combn_base, "data_out", "flowsom", 'flowsom.rds')
    if(file.exists(path_flowsom_old)) out_list$flowsom <- readRDS(path_flowsom_old)
  }
  ...
  if(compass){
    ...
    compass_list <- purrr::map(gate_name_vec, function(gate_name_curr){
      ...
      purrr::map(compass_stim, function(stim_curr){
        ...
        stats_combn_tbl_curr <- stats_combn_tbl %>%
          dplyr::filter(gate_name == gate_name_curr, stim == stim_curr)
        ...
        full_tbl <- stats_combn_tbl_curr %>%
          dplyr::select(SubjectID, VisitType, stim, gate_name, cyt_combn, n_cell_stim,
                 count_stim, n_cell_uns, count_uns) %>%
          dplyr::left_join(TuberculomicsCompendium::clinical_data %>%
                      dplyr::select(SubjectID, Progressor, timeToTB, timeToTBFromVisit, SampleID, VisitType),
        ...
        data_uns <- full_tbl %>%
          dplyr::select(SampleID, cyt_combn, count_uns, n_cell_uns) %>%
          dplyr::rename(count = count_uns, n_cell = n_cell_uns)
        ...
        if(!identical(rownames(data_stim), rownames(data_uns))) stop("stim and uns not identically ordered")

        data_meta <- full_tbl %>%
          dplyr::select(SubjectID, VisitType, SampleID, gate_name, stim, timeToTB, Progressor) %>%
          dplyr::group_by(SampleID, SubjectID, VisitType, gate_name, stim, timeToTB, Progressor) %>%
          dplyr::slice(1) %>%
          dplyr::ungroup() %>%
          as.data.frame()

        data_meta <- purrr::map_df(rownames(data_stim), function(id){
          data_meta %>% dplyr::filter(SampleID == id)
        })

        if(!identical(data_meta$SampleID, rownames(data_uns))) stop("meta and uns not identically ordered")

        # =============================
        # Run COMPASS
        # =============================

        fit <- COMPASS::SimpleCOMPASS(n_s = data_stim,
                                      n_u = data_uns,
                                      meta = data_meta,
                                      individual_id = "SampleID",
                                      iterations = ifelse(compass_fast, 1e2, 4e4),
                                      replications = ifelse(compass_fast, 2, 8))
    ...
    analysispipeline::save_objects('compass' = compass_list,
                                   dir_proj = path_cyt_combn_base,
                                   dir_sub = c('data_out', 'compass'),
                                   empty = TRUE)

    out_list %<>% append(list('compass' = compass_list))
  }
  ...
  # get exclusion options for downstream t-SNE, post probs and flowSOM (as requested)
  if(is.null(downstream_exc_list)){ # if not specified, just take files already available
    exc_vec <- list.dirs(path_cyt_combn_base, full.names = FALSE, recursive = FALSE)
    ...
    exc_vec <- purrr::map_chr(downstream_exc_list, function(downstream_exc){
      if(is.null(downstream_exc)) return("exc-none")
  ...
  if(length(exc_vec) == 0){
    ...
    eval(parse(text = paste0('usethis::use_data(', ds_name,
                             ', overwite = TRUE)')))
  ...
  if(tsne || flowsom){
    ...
    ex_ag_list <- purrr::map(exc_vec, function(exc){
      ...
      if(!dir.exists(dir_fcs_exc)){
        warning(paste0(dir_fcs_exc, " does not exist for getting cyt+ expression data"))
        return(NULL)
      }
      gn_vec <- list.dirs(dir_fcs_exc, full.names = FALSE, recursive = FALSE)
      if(length(gn_vec) == 0){
        if(!dir.exists(dir_fcs_exc)){
          warning(paste0("No sub-directories in ", dir_fcs_exc, " for getting cyt+ expression data"))
          return(NULL)
        }
      }
      purrr::map(gn_vec, function(gn){
        ...
        if(!dir.exists(dir_fcs_exc_gn_fcs)) return(NULL)
        ...
        if(length(fcs_vec) == 0) return(NULL)
        ex_tbl <- purrr::map(fcs_vec, function(x){
          ex <- try(flowCore::exprs(flowCore::read.FCS(file.path(dir_fcs_exc_gn_fcs, x))) %>%
                      tibble::as_tibble() %>%
                      dplyr::mutate(fcs = x))
          if(identical(class(ex), 'try-error')) return(NULL)
          ex
        ...
        sample_info_tbl <- purrr::map_df(unique(fcs_vec), function(fcs){
          ...
          tibble::tibble(SubjectID = split_vec[1],
    ...
    # ------------------
    df_lab <- flowCore::parameters(flowWorkspace::gh_pop_get_data(gs[[1]]))@data
    chnl_vec_sel <- c('Er166Di', 'Ho165Di', 'Gd155Di', 'Dy163Di', 'Dy164Di',
    ...
    tsne_list <- purrr::map(names(ex_ag_list), function(exc){
      purrr::map(names(ex_ag_list[[exc]]), function(gn){
        if(!tsne_reuse) return(NULL)
        path_tsne_old <- file.path(path_cyt_combn_base, exc, gn,
                                   "data_int", 'tsne', 'tsne.rds')
        if(!file.exists(path_tsne_old)) return(NULL)
    ...
    tsne_list <- purrr::map(exc_vec, function(exc){
      gn_vec <- names(ex_ag_list[[exc]]); gn <- gn_vec[1]
      purrr::map(gn_vec, function(gn){
        ...
        if(tsne_rerun){
          ...
          for(cn in colnames(ex_tbl)){
            if(!cn %in% marker_vec_sel) ex_tbl <- ex_tbl[,-which(colnames(ex_tbl) == cn)]
          }
          ...
          tsne_tbl <- ex_ag_list[[exc]][[gn]] %>%
            dplyr::mutate(tsne1 = tsne_mat[,1],
                          tsne2 = tsne_mat[,2],
                          umap1 = umap_mat[,1],
                          umap2 = umap_mat[,2])

          analysispipeline::save_objects('tsne' = tsne_tbl,
                                         dir_proj = path_cyt_combn_base,
        ...
        if(tsne_plot_final){
          ...
          if(!tsne_plot) return(tsne_tbl)

          p_marker_tsne <- ggplot(plot_tbl %>%
          ...
          analysispipeline::save_objects(p_marker_tsne = p_marker_tsne,
                                         dir_proj = path_cyt_combn_base,
          ...
          analysispipeline::save_objects(p_marker_umap = p_marker_umap,
                                         dir_proj = path_cyt_combn_base,
                                         dir_sub = c(exc, gn, 'tsne'),
                                         height = 45,
                                         width = 50,
          ...
          p_prog_stim_equal_cells <- ggplot(tsne_tbl %>%
          ...
          p_prog_stim_raw <- ggplot(tsne_tbl %>%
                                      dplyr::select(fcs:tsne2),
                                    aes(x = tsne1, y = tsne2)) +
            coord_equal() +
            geom_hex(bins = 32) +
          ...
          p_prog_stim_equal_cells_hex_only <- ggplot(tsne_tbl %>%
                                                       dplyr::select(fcs:tsne2),
                                                     aes(x = tsne1, y = tsne2)) +
            coord_equal() +
            geom_hex(bins = 32,
          ...
          p_prog_stim_raw_hex_only <- ggplot(tsne_tbl %>%
                                               dplyr::select(fcs:tsne2),
                                             aes(x = tsne1, y = tsne2)) +
            coord_equal() +
            geom_hex(bins = 32) +
          ...
          p_prog_stim_dens_only <- ggplot(tsne_tbl %>%
                                            dplyr::select(fcs:tsne2),
                                          aes(x = tsne1, y = tsne2, col = Progressor)) +
            coord_equal() +
            geom_density_2d(size = 0.5) +
            scale_colour_manual(values = c("Progressor" = "orange2",
                                           "Control" = "dodgerblue")) +
            facet_wrap(Progressor ~ stim, ncol = 4) +
            theme(axis.text = element_blank(),
                  axis.ticks = element_blank()) +
          ...
          analysispipeline::save_objects(p_prog_stim_dens_only = p_prog_stim_dens_only,
                                         p_prog_stim_raw_hex_only = p_prog_stim_raw_hex_only,
                                         p_prog_stim_raw = p_prog_stim_raw,
                                         p_prog_stim_equal_cells_hex_only = p_prog_stim_equal_cells_hex_only,
          ...
          p_list_save <- list(p_prog_stim_dens_only = p_prog_stim_dens_only,
                              p_prog_stim_raw_hex_only = p_prog_stim_raw_hex_only,
                              p_prog_stim_raw = p_prog_stim_raw,
                              p_prog_stim_equal_cells_hex_only = p_prog_stim_equal_cells_hex_only,
                              p_prog_stim_equal_cells = p_prog_stim_equal_cells)
    ...
    if(tsne_save) analysispipeline::save_objects('tsne' = tsne_list,
                                                 dir_proj = path_cyt_combn_base,
  ...
  if(flowsom || post_probs_bulk){
    ...
    responders_list <- purrr::map(exc_vec, function(exc){
      ...
      if(exc != 'exc-none'){
        ...
        cyt_vec <- purrr::map_chr(cyt_vec, function(x){
          if(!stringr::str_sub(x, end = 1) == "!") return(x)
          stringr::str_sub(x, start = 2)
        })
        marker_lab_vec_cyt <- setNames(names(chnl_lab_vec), chnl_lab_vec)
        chnl_vec <- marker_lab_vec_cyt[cyt_vec]

        cyt_combn_exc_vec <- purrr::map_chr(chnl_combn_exc_vec_raw, function(x){
          ...
          purrr::map_chr(chnl_vec, function(chnl){
            if(chnl %in% chnl_vec_exc) return(
      ...
      } else cyt_combn_exc_vec <- NULL



      dir_fcs_exc <- file.path(path_cyt_combn_base, exc)
      gn_vec <- list.dirs(dir_fcs_exc, full.names = FALSE, recursive = FALSE)
      if(length(gn_vec) == 0) return(NULL)
      gn <- gn_vec[1]
      purrr::map(gn_vec, function(gn){
    ...
    out_list %<>% append(list('post_probs_bulk' = responders_list))
  }
  ...
  if(flowsom){
      ...
      for(exc in exc_vec){
        ...
        for(gn in gn_vec){
          ...
          for(stim in flowsom_stim){
            ...
            responders_list_resp_stim <- switch(
              as.character(responders_only == 'r_o'),
              "FALSE" = NULL,
            ...
            for(scale in flowsom_scale){
              ...
              for(n_clust in flowsom_n_clust){
                ...
                dir_save_base <- file.path(str_sub(dir_fcs_exc_gn_fcs, end = -5),
                                           'fs', responders_only)
                ...
                if(!dir.exists(dir_save_cluster_results)) dir.create(
                  dir_save_cluster_results, recursive = TRUE
                  )
                ...
                } else flowsom_cluster_post_probs <- NULL
                ...
                flowsom_tbl_curr <- flowsom_freq %>%
                  ...
                  dplyr::mutate(responders_only = responders_only, exc = exc, gn = gn,
                         stim = stim, scale = scale, n_clust = n_clust)

                if(!is.null(flowsom_cluster_post_probs)){
                  flowsom_tbl_curr %<>%
                    dplyr::left_join(flowsom_cluster_post_probs, by = c("clust", "stim", "SubjectID", "VisitType"))
                } else flowsom_tbl_curr %<>% dplyr::mutate(prob = NA)

                flowsom_tbl_curr %<>%
                  dplyr::select(responders_only:n_clust, stim,
                         clust, stability,
                         SubjectID, VisitType,
                ...
                if(stim %in% c("all", "all_u")) flowsom_tbl_curr %<>% dplyr::mutate(stim = paste0(.env$stim, "-", stim))

                # output
                if(!exists('flowsom_tbl')){
                  flowsom_tbl <- flowsom_tbl_curr
                } else{
                  flowsom_tbl %<>% dplyr::bind_rows(flowsom_tbl_curr)
                }
              }
            }
          }
    ...
    out_list %<>% append(list('flowsom' = flowsom_tbl))
  }
  ...
  # prep
  dir_data_rda <-   file.path(here::here(), "data", paste0(ds_name, ".rda"))
  if(!dir.exists(dir_data_rda)){
    dir.create(dir_data_rda)
  }
  envir <- environment()
