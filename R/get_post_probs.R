...
get_post_probs <- function(stats_combn_tbl, stim, exc,
  ...
  if(is.null(chnl_lab_cyt_custom)) chnl_lab_cyt_custom <- c('Ho165Di' = 'IFNg', 'Gd158Di' = 'IL2', 
  ...
  out_list <- purrr::map(stim, function(stim_ind){
    ...
    ds <- stats_combn_tbl %>%
      dplyr::filter(.data$stim == stim_ind, 
             gate_name == gn)
    ...
    exc_cyt_single_vec <- purrr::map(stringr::str_split(exc, "_")[[1]], function(x){
      ...
      for(chnl in chnl_vec) if(str_detect(x, chnl)) return(chnl)
      NULL
    }) %>%
      purrr::compact() %>%
      unlist()
    ...
    cyt_lab_cyt_vec <- setNames(names(chnl_lab_cyt_custom), 
                            chnl_lab_cyt_custom)
    ...
    exc_cyt_combn_vec <- purrr::map_chr(exc_cyt_single_vec, function(x){
      if(x == chnl_vec[1]) return(paste0(chnl_vec[1], "&!", 
    ...
    ds %<>%
      group_by(gate_name, SampleID, SubjectID, VisitType, stim, n_cell_stim,
               n_cell_uns) %>%
      dplyr::summarise(count_stim = sum(count_stim),
    ...
    ds %<>%
      ...
      dplyr::mutate(count_neg = ifelse(stim == 'uns',
    ...
    # browser()
    #library(MIMOSA)
    mimosa_expr_set <- MIMOSA::ConstructMIMOSAExpressionSet(thisdata = ds,
    ...
    mim_res <- MIMOSA::MIMOSA(formula = count_neg + count_pos ~ SampleID|feature,
    ...
    list('post_prob' = post_prob_lab_vec, 
